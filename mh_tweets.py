...
def tweets_to_db(query, db_name):
        ...
        #Get the tweets
        tweets = tweepy.Cursor(api.search, q=query+' -filter:retweets -filter:replies', lang='en').items(1250)
        ...
        for tweet in tweetsArray:
            ...
            readScaleVal.append(textstat.flesch_reading_ease(tweet))

        # Create pandas dataframe from arrays
        tweet_frame = pd.DataFrame({"Tweets":tweetsArray, "TimeStamp":timestampArray, "UTF_Offset":utfArray, "Polarity":polarityVal, "Subjectivity":subjectivityVal, "Readability":readScaleVal}, columns=["Tweets", "TimeStamp", "UTF_Offset", "Polarity", "Subjectivity", "Readability"])

        # Connect to Heroku and write to db
        engine = create_engine(tweet_db_url)
        tweet_frame.to_sql(db_name, con=engine, if_exists='replace')

        pass

tweets_to_db('mental health', 'mh_tweets')
tweets_to_db('mental illness', 'mi_tweets')
tweets_to_db('brain disease', 'bd_tweets')
tweets_to_db('brain health', 'bh_tweets')
